...
export function LearningHome() {
  ...
  return (
    <div className="w-full min-w-0 max-w-full">
      <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Learning module</h1>
      ...
      <div className="mt-6 grid gap-3 sm:mt-8 sm:grid-cols-2 sm:gap-4">
        {cards.map(({ to, title, titleUr, desc, icon: Icon }) => (
          <Link
            key={to}
            to={to}
            className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-brand-300 sm:p-6"
          >
