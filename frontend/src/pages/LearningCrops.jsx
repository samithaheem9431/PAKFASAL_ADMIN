...
export function LearningCrops() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (fromCache) {
        toast(`You're offline — showing crops cached from ${formatCacheTimestamp(cachedAt)}`, {
          icon: "📴",
        });
      }
    } catch (e) {
  ...
  const remove = async (id) => {
    if (!confirm("Delete this crop? Diseases referencing it will remain orphaned.")) return;
    try {
    ...
    <div className="w-full min-w-0 max-w-full">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Crops</h1>
          <p className="text-slate-600">Manage the "Keera aur Bimariyan" crop list</p>
        </div>
        <Link
          to="/learning/crops/new"
